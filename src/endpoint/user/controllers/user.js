...
const update_user = async (req, res) => {
    ...
    var payload = [username, encrypted_password, nama, nik, jenis_kelamin, no_hp, alamat, pekerjaan, no_rekening, status_perkawinan, email, id_user]

    const token = raw_token.split(' ')[1]

    const query = 'UPDATE user SET username = ?, password = ?, nama = ?, nik = ?, jenis_kelamin = ?, no_hp = ?, alamat = ?, pekerjaan = ?, no_rekening = ?, status_perkawinan = ?, email = ? WHERE id_user = ?'
