...
const simpanan_parent_list = async (req, res) => {
    ...
    const query = `SELECT MAX(simpanan.id_nasabah), user.nama, SUM(simpanan.nominal), simpanan.tahun FROM simpanan INNER JOIN user ON user.id_user=simpanan.id_nasabah WHERE tipe_simpanan = 'Wajib' GROUP BY simpanan.tahun, user.nama`
